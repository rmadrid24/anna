...
nvmmiddleware::NvmMiddleware::NvmMiddleware(string dbPath, int interactiveThreads, int batchThreads)
{
...
void nvmmiddleware::NvmMiddleware::start_db(string dbPath)
{
    pmem::kv::config cfg;
...
pmem::kv::status nvmmiddleware::NvmMiddleware::pmemkv_put(pmem::kv::db *db, const string *k, const string *value)
{
    auto start_time = chrono::system_clock::now();
    auto s = db->put(*k, *value);
...
pmem::kv::status nvmmiddleware::NvmMiddleware::pmemkv_get(pmem::kv::db *db, const string *k, string *reply)
{
    auto start_time = chrono::system_clock::now();
    auto s = db->get(*k, reply);
    auto end_time = chrono::system_clock::now();
    avg_get_pmemkv += calculate_time_ms(start_time, end_time);
    if (s != pmem::kv::status::OK)
    {
        std::cout << "Error: key " << *k << " message " << s << std::endl;
    }
    return s;
}
